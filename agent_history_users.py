...
1. Analiza el requerimiento del usuario. Clasifícalo mentalmente en una de dos opciones:
   - **Claro y Suficiente**: Si contiene suficiente detalle de flujos, reglas de negocio o contexto para redactar la HU final inmediatamente sin necesidad de más preguntas.
   - **Vago o Insuficiente**: Si es corto, ambiguo, incompleto o carece de contexto funcional claro (ej. "quiero un botón de pago").

2. Si el requerimiento es **Claro y Suficiente**:
...
3. Si el requerimiento es **Vago o Insuficiente**:
...
def run_refinement(raw_input: str, provider: str, model_name: str, api_key: str) -> str:
    ...
    prompt = f"""{SYSTEM_ANALYSER_PROMPT}
...
def run_story_writer(input_data: str, provider: str, model_name: str, api_key: str) -> str:
    llm = get_llm(provider, model_name, api_key)
    if input_data.startswith("Generate story from:"):
        input_data = input_data[len("Generate story from:"):].strip()
        
    prompt = f"""{SYSTEM_WRITER_PROMPT}
...
class DynamicExecutor:
    def __init__(self, run_func, provider: str, model_name: str, api_key: str):
    ...
    def invoke(self, inputs):
        ...
        output = self.run_func(input_val, self.provider, self.model_name, self.api_key)
